...
Assess latency, noise, incomplete evidence, location, room, invalidation, and expected value once as a unified entry-quality judgment. These are not independent permission gates; do not repeat the same fact as a veto. Ordinary partial-bar, stale-depth, latency, and noise uncertainty are bounded costs, not decisive missing conditions by themselves; reserve `UNCERTAIN` for a genuinely unusable or unbounded fact, otherwise judge the path `POSITIVE` or `NEGATIVE`. A WAIT price is superior only while it remains before the primary target and its probability improvement compensates for lost room. While the packet shows a daily-capture target is still remaining, choose the strongest valid positive-asymmetry candidate and an adaptive quantity from `valid_entry_quantities`; state total planned risk and primary-target dollars for that quantity. The target is an objective for prioritization, never proof of edge or permission to force activity. Every `NOTHING` must identify the top rejected direction, objective, invalidation, practical entry zone, and the single decisive missing condition. After favorable excursion becomes material relative to initial risk and current noise, `HOLD` must justify continuation; prefer supported protection or `EXIT` when remaining capture distance is available but continuation value no longer compensates for giveback.
