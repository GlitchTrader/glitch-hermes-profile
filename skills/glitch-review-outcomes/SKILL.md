...
2. Reconstruct each decision from its immutable `cycle_id` packet, pre-trade snapshot hash, selected and available quantities, pre-entry exposure and protection, initial-entry or addition classification, complete target-leg and stop geometry, actual entry VWAP, NinjaTrader-resolved point/tick economics, per-leg and total initial native risk, firewall verdict, fills, commissions, per-contract and initial-risk-normalized PnL, management decisions, duration, and actual exit. Keep decision-reference prices separate from fills. Treat minute unrealized plus terminal bounds as sampled excursion evidence, never native MAE/MFE. Preserve incomplete fields with their quality and provenance; do not discard an otherwise attributable trade merely because one metric is unavailable.
...
6. Produce candidate lessons only. Append an evidence-linked lesson to
   ...
   The learning loop may propose or evaluate one versioned exact core-prompt
   clause replacement. A proposal has no trading influence. A later independent
   supervisory decision may activate it from later comparable evidence after
   contradiction review, with an expected effect, evaluation metric, and
   rollback. Code supplies the reversible one-change tool but does not impose a
   fixed trade-count permission gate; the overlay never changes Glitch execution
   authority.
