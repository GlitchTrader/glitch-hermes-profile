...
- Optional leg 2: `take_profit_2`, `quantity_tp1`, and optional `stop_loss_2`. Optional leg 3: `take_profit_3`, `quantity_tp2`, and optional `stop_loss_3`. The remaining quantity runs to the last target. Targets progress farther in the profit direction; optional later stops remain on the loss side and may only be tighter. These native entry legs are the current scale-out mechanism; the action contract has no discretionary partial-reduction action after entry.
