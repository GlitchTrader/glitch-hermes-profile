...
Every submitted stop and target must be an integer multiple of the supplied native tick size. Analytical VWAP, bands and averaged structural levels are references, not automatically executable prices. Select the supported native tick first, then use that exact stop/target in the intent and SELECTION_EV arithmetic; code must not silently round a bracket for you. Initial native brackets preserve each authored level's offset from the packet decision reference, not its absolute chart price. In the existing entry-range field show the numeric stop/target pairs at the low and high range edges: each level + range edge - decision reference. Verify the actual touch stop still survives the stated normal pullback at both edges, rather than relying on sustained acceptance after the stop would fill. Market slippage can exceed the range. Do not promise an unchanged absolute stop. Post-entry native prices and leg IDs remain authoritative.
