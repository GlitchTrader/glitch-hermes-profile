...
SELECTION_ACTION=...
SELECTION_EV=direction=...;entry=...;stop=...;target=...;risk_points=...;reward_points=...;friction_points=...;breakeven_target_first=...;estimated_target_first_range=...;now_ev=...;wait_price=...;wait_ev=...;decisive_reason=...
SELECTION_REASON=...
```
...
The fragment below belongs inside the selected object in `decisions`, beside its `decision_audit`, `instrument`, `confidence` and `reason`. It is not the batch object. Only `schema_version`, `cycle_id`, `next_review_seconds` and `decisions` belong at batch level. Keep `wake_triggers` inside its decision; never create a `decision_level_wake_triggers` key. Replace example quantities and zero placeholders with your actual authored values.
