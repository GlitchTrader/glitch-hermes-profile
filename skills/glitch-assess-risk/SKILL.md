...
4. An existing same-direction, fully protected position may permit `HOLD`, `MOVE_STOP`, `MOVE_TP`, `EXIT`, or another protected tranche. An addition may occur at a favorable or adverse price only when current evidence still supports the thesis; never add merely because price moved against the position or to recover a loss. `MOVE_TP` may carry a tighter stop but can never loosen it. Never reverse through an entry or exceed supplied capacity.
