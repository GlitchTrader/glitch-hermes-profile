...
A sweep/reclaim, failed acceptance, displacement, absorption, exhaustion, or microstructure break can transition the state. A break weakens or invalidates the affected path and may promote the next setup for reassessment; it does not force a reverse. At a session extreme or stated first objective, one delta change or one-minute bounce does not promote the opposing path without accepted price response through its named transition. A farther objective requires present evidence for continuation, but need not already have traded; distinguish intermediate response levels from the primary destination. Preserve anticipatory entries when location is favorable, invalidation is genuine, credible room remains, and probability-weighted asymmetry after latency is positive. The entry range bounds acceptable fills during delivery; it is not a second stop and need not span an ATR. Judge stop survival from genuine invalidating structure and observed adverse movement over the intended trade horizon. One- and five-minute ATR are context, not cumulative clearance requirements. A shallow pivot that only creates a small risk denominator is not sufficient invalidation. Do not require a closed candle, complete flow, or false numeric precision solely to eliminate uncertainty.

Keep the parent auction, entry trigger and local attempt separate. A trigger or promotion level is neither automatically the target nor the hard-stop boundary. Choose the wager's horizon first: a local attempt has its own destination and touch-stop probability; a parent rotation must survive the pullbacks/probes that leave that parent intact. Do not combine the parent's confidence or distant objective with a minute-level stop that rejects those pullbacks. Acceptance failure and stop-on-touch are different events. Derive a supported touch boundary beyond the allowed probes, using observed extremes and adverse legs, rather than labeling one tick beyond a trigger as structural. These are interpretations of supplied evidence, not required pre-labeled fields or mandatory higher-timeframe alignment. When current location makes coherent risk expensive, assess a better entry location before demanding additional confirmation at a worse price.
