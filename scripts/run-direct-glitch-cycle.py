...
def position_management_template(book: dict[str, Any]) -> str:
    ...
    for field in POSITION_MANAGEMENT_FIELDS:
        if field == "CURRENT_SETUP":
            lines.append(
                "CURRENT_SETUP=REPLACE_WITH_HELD_OR_FAILED_COLON_CURRENT_EVIDENCE"
            )
        elif field == "HOLD_EV":
            lines.append(
...
def contract_repair_prompt(
    ...
    if error_text.startswith("position_management_thesis_status_invalid:"):
        return (
            ...
            "HELD: or FAILED:. FAILED is valid only when PREVIOUS_RESPONSE already states that "
            "the entry-authored invalidation was materially satisfied or names a specific accepted "
...
def build_prompt(
    ...
    if positioned_only:
        instructions = (
            ...
            "requests are not confirmed changes. Reconcile each leg's current native state, including failed or partial results. "
            "Reassess now; do not blindly retry or forget its motive. "
        ...
        instructions = (
            ...
            "An earlier target touch completes the OLD forecast, not permanently removes that price as a destination. "
            "After price moves away, assess a fresh return-to-level or extension path from the current entry and response; "
            "a newly printed farther level is not required. Previously touched does not mean zero current room. "
            "New probabilities start now: never count the earlier touch as success or revive the old order/forecast. "
