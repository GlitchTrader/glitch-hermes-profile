...
def build_prompt(
    ...
    entry_continuity = (
        "For a flat entry decision, recent_glitch_ledger.recent_exit_decisions and completed native results take precedence over the pre-exit thesis only as factual continuity for that specific exited auction path; they are not a general penalty on an instrument or direction. Treat the exit rationale and its change_condition as the continuity baseline; do not carry the exited setup forward as HELD merely because its old objective or broad invalidation remains open. Recent completed native entry attempts are correlated evidence, not independent trials, only while they substantially share the same instrument, direction, objective, invalidation, structural location, and auction path. Use adverse market behavior observed before and at the exit as evidence against that unchanged path; realized P&L, a win/loss label, and the fact of exit are not market evidence. Same instrument and direction or a broad objective alone do not make a new setup correlated. A completed new leg, break-and-hold, or pullback/retest can make the setup distinct only when it materially relocates entry or changes invalidation, auction state, or target-first probability; then evaluate it from current evidence. NOTHING, HOLD, trigger reviews, rejected candidates, and opposite-direction trades are observations, not attempts on that thesis; their labels or presence in history cannot by themselves lower its probability, though their underlying current-market evidence still matters. Flatness, elapsed time, a new bar, recross, delta change, better price, attractive payoff geometry, or remaining daily-capture progress is not by itself material post-exit evidence. Reward/risk and a low break-even probability are payoff math, not probability evidence. Re-enter immediately when concrete evidence observed after the exit creates a genuinely distinct positive-EV setup; otherwise choose NOTHING. This is cognitive continuity, not a cooldown or deterministic execution gate. "
    ...
    if not positioned_only:
        instructions += (
            ...
            "Use each candidate's deterministic_geometry_context as the arithmetic authority for tick value, one- and five-minute ATR, spread, and their one-contract dollar equivalents; it is decision support, not a setup signal or execution gate. Compare candidates in one-contract dollars as well as points and ticks. Greater dollar noise or spread is not a veto, but its evidence-supported probability and reward must compensate for the larger excursion and estimation error; when otherwise comparable, prefer the candidate with less dollar downside. For WAIT geometry, lower is a price improvement for a long and higher is a price improvement for a short. A worse-price confirmation may still improve probability, but call it better only when that probability gain outweighs the lost reward and increased invalidation cost. It is an internal arithmetic contradiction to write now_ev=NEGATIVE when the entire estimated target-first range is above exact break-even, or now_ev=POSITIVE when the entire range is below it. If qualitative evidence disagrees with that implication, revise the estimated range from the evidence; never preserve both contradictory claims. A straddling range remains an evidence judgment. This consistency rule does not choose probability, geometry, instrument, or action. "
        )
